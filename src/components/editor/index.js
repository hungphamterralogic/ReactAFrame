...
const Editor = props => {
  ...
  useEffect(() => setRotation(), [props.position]);
  ...
  const renderHotspotRow = h => (
    <ExpansionPanel key={h.id}>
      ...
      <ExpansionPanelDetails>
        <Typography>
          ...
          <button type="button" onClick={() => onDeleteHotspot(h.id)}>
            Delete
          </button>
        </Typography>
      </ExpansionPanelDetails>
  ...
  return (
    <div className={c(classes.editor, classes.open)}>
      ...
      {`Index:${screenId} `}
      <br />
      {`ID: ${getScreenByIndex(screenId).id} `}
      <br />
      {`Name: ${getScreenByIndex(screenId).name} `}
      <br />
      {`hotspots:`}
      ...
      {isOpenAdd && (
        <div className={classes.addhotspot}>
          ...
          <select onChange={onChangeType}>
            <option checked={type === 'info'} value="info">
              info
            </option>
            <option checked={type === 'teleport'} value="teleport">
              teleport
            </option>
          </select>
          <br />
          {type === 'info' ? (
            ...
            <span>
              ...
              <select onChange={e => setNextId(e.target.value)}>
                {metadata.map(s => (
                  <option key={s.id} checked={type === s.id} value={s.id}>
                    {s.name}
                  </option>
                ))}
              </select>
          ...
          <div className={classes.flex}>
            <div className={c(classes.button)} onClick={handleApplyHotspot}>
              Apply Hotspot
            </div>
            ~
            <div className={c(classes.button)} onClick={handleCloseAdd}>
              Close
            </div>
          </div>
          <hr />
      ...
      <div className={classes.tool}>
        {!isOpenAdd && (
          <div className={classes.button} onClick={handleClickAdd}>
            {`Add hotspot`}
          </div>
        )}
      </div>
      ...
      <div className={classes.bottom}>
        {metadata.map(s => (
          <div
            key={s.id}
            className={c(classes.screenItem, {
              [classes.selectedScreen]: s.id === getScreenByIndex(screenId).id
            })}
